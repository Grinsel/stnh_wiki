...
export function renderForceDirectedArrowsGraph(
  ...
  // Polygon point calculator — shared between tick handler and post-worker call
  function arrowPoints(d) {
    ...
    const px = -ny, py = nx;
    const baseWidth = 10;
    const halfWidth = nodeWidth / 2, halfHeight = nodeHeight / 2;
    const absDx = Math.abs(dx), absDy = Math.abs(dy);
    let tipX, tipY;
    if (absDx / halfWidth > absDy / halfHeight) {
      const s = Math.sign(dx);
      tipX = d.target.x - s * halfWidth;
      tipY = d.target.y - (s * halfWidth * dy) / dx;
    } else {
      const s = Math.sign(dy);
      tipY = d.target.y - s * halfHeight;
      tipX = d.target.x - (s * halfHeight * dx) / dy;
    }
    return `${tipX},${tipY} ${d.source.x + px * baseWidth / 2},${d.source.y + py * baseWidth / 2} ${d.source.x - px * baseWidth / 2},${d.source.y - py * baseWidth / 2}`;
  }
  ...
  function boundingForce() {
    for (const n of nodes) {
      ...
      if (dist > maxDistance) {
        n.vx += (center.x - n.x) * 0.01 * simulation.alpha();
        n.vy += (center.y - n.y) * 0.01 * simulation.alpha();
      }
    }
  }
  ...
  runForceInWorker(nodeSnaps, linkSnaps, width, height, {
    ...
    const linksSel = _g.select('.links-layer').selectAll('polygon')
      .data(links).join('polygon')
      .attr('class', 'link')
      .attr('fill', '#e0e0e0ff').attr('stroke', 'black')
    ...
    node.append('rect').attr('class', 'node-rect')
      ...
      .attr('fill', (d) =>
        d.area === 'society' || d.area === 'engineering' || d.area === 'physics'
          ? `url(#gradient-${d.area})` : getAreaColor(d.area))
      .attr('stroke', (d) => {
