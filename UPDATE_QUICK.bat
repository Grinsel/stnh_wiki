...
set LOGFILE=%~dp0logs\UPDATE_QUICK_%D%_%T%.log
cmd /c "%~f0" 2>&1 | powershell -c "$input | Tee-Object -FilePath '%LOGFILE%'"
exit /b %ERRORLEVEL%
:skip_logging

echo ============================================
echo  STNH Wiki - Quick Update (no images)
echo ============================================
echo.
...
echo [1/3] Running UPDATE_WIKI.py --skip-images ...
echo.
cd /d "%~dp0update"
...
git commit -m "Update STNH Wiki (quick) - %date% %time:~0,8%%PIPELINE_WARN%"
if %ERRORLEVEL% neq 0 goto :commit_failed
