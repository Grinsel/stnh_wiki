...
def phase_verify_models():
    """Phase: Model consistency check (read-only, warn-only).

    Cross-checks the referenced ship/mega GLBs against the files on disk.
    Has no skip parameter on purpose: it must run even with --skip-images so a
    fast update still reports missing models.
    """
    print("\n" + "=" * 60)
    print("PHASE: MODEL CONSISTENCY CHECK")
    print("=" * 60)

    from verify_ship_models import verify_models
...
def write_build_version():
    """Write js/build-version.js with a fresh cache-busting build timestamp.

    Runs on every invocation (including --only runs) so the frontend can detect
    a new deploy. Generated file; do not edit it by hand.
    """
    from config import WIKI_ROOT
    version = datetime.now().strftime('%Y%m%d%H%M%S')
    path = os.path.join(WIKI_ROOT, 'js', 'build-version.js')
    with open(path, 'w', encoding='utf-8') as f:
        f.write("// Generated by UPDATE_WIKI.py - do not edit\n")
        f.write(f"window.WIKI_BUILD_VERSION = '{version}';\n")
    print(f"  Build version written to: {path} ({version})")
...
ONLY_MODULES = {
    ...
    'ship_models': ['localisation', 'inject_missing_loc', 'ships', 'ship_models', 'verify_models', 'split_loc'],
    ...
    'mega_models':    ['localisation', 'inject_missing_loc', 'megastructures', 'mega_models', 'verify_models', 'split_loc'],
    ...
    'content':        ['localisation', 'inject_missing_loc', 'ships', 'buildings', 'all_icons', 'traits',
                       ...
                       'resources', 'search', 'mega_models', 'verify_models', 'split_loc'],
}
...
def main():
    ...
    PHASES = [
        ...
        ('verify_models',      phase_verify_models),
    ...
    # Write cache-busting build version on every run (incl. --only).
    write_build_version()
    ...
    # Model consistency check summary (warn-only, never affects exit code).
    vm = results.get('verify_models')
    if isinstance(vm, dict) and not vm.get('skipped'):
        if vm.get('ok'):
            print("  [MODEL CHECK] OK")
        else:
            print(f"  [MODEL CHECK] {vm.get('ship_missing', 0)} ship GLB(s) missing, "
                  f"{vm.get('mega_missing', 0)} mega GLB(s) missing (warn-only)")
    elif isinstance(vm, dict) and vm.get('skipped'):
        print("  [MODEL CHECK] skipped (ships.json not found)")
